...
def save_artifacts(
    ...
    result: dict,
    test_name: str,
    artifacts_dir: str = "compression_runs"
):
    """Save original, compressed, and decompressed files to a timestamped folder."""
    timestamp = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    label = test_name.lower().replace(" ", "_")
    folder = os.path.join(artifacts_dir, f"{timestamp}_{label}")
    ...
    # compressed binary
    with open(os.path.join(folder, "compressed.bin"), "wb") as f:
        f.write(compressed)

    # decompressed text
    with open(os.path.join(folder, "decompressed.txt"), "w", encoding="utf-8") as f:
        f.write(decompressed)

    # diff check
    if text == decompressed:
        diff_status = "IDENTICAL — perfect lossless reconstruction"
    else:
        diff_status = f"MISMATCH — length original={len(text)} decompressed={len(decompressed)}"
        for idx, (a, b) in enumerate(zip(text, decompressed)):
            if a != b:
                diff_status = f"MISMATCH at char {idx}: original={repr(a)} decompressed={repr(b)}"
                break
    ...
    meta = {
        "label": test_name,
        "timestamp": timestamp,
        "model": result.get("model", "distilgpt2"),
        "original_size_bytes": result["original_bytes"],
        "llm_zstd_size_bytes": result["llm_zstd_size"],
        "llm_zstd_ratio_percent": round(result["llm_zstd_ratio"], 2),
        "zlib_size_bytes": result["zlib_size"],
        "zlib_ratio_percent": round(result["zlib_ratio"], 2),
        "zstd_only_ratio_percent": round(result["zstd_only_ratio"], 2),
        "compression_tps": round(result["llm_tps"], 2),
        "decompression_tps": round(result["decompress_tps"], 2),
        "token_count": result["token_count"],
        "lossless_verified": text == decompressed,
        "diff_status": diff_status,
        "llm_beats_zlib": result["llm_zstd_ratio"] < result["zlib_ratio"],
    }
    with open(os.path.join(folder, "run_info.json"), "w") as f:
        json.dump(meta, f, indent=2)

    print(f"\n📁 Artifacts saved to: {os.path.abspath(folder)}/")
    print(f"   original.txt     ({result['original_bytes']} bytes)")
    print(f"   compressed.bin   ({result['llm_zstd_size']} bytes)")
    print(f"   decompressed.txt")
...
def benchmark(
    ...
    # Save artifacts
    if save:
        save_artifacts(text, compressed, decompressed, result, test_name, artifacts_dir)
        ...
        ("JSON Data", """
{
  ...
  "products": [
    ...
    for name, text in test_cases:
        result = benchmark(text, compressor, name, save=True, artifacts_dir="compression_runs")
    ...
    print(f"\n📁 All artifacts saved to: {os.path.abspath('compression_runs')}/")
